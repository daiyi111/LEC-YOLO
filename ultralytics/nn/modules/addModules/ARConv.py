...
class ARConv(nn.Module):
    ...
    def forward(self, x, epoch=100, hw_range=[1,18]):
        ...
        l = self.l_conv(offset) * (hw_range[1] - 1) + 1  # b, 1, h, w 范围0-1
        ...
        # 最终的总采样点数目
        N = N_X * N_Y
        # print(N_X, N_Y)  l和w是动态生成的长度和宽度
        l = l.repeat([1, N, 1, 1])  # (b, N, h, w)
        w = w.repeat([1, N, 1, 1])  # (b, N, h, w)
        offset = torch.cat((l, w), dim=1)  # (b, 2*N, h, w)
...
def main():
    ...
    # 生成随机输入张量（假设输入为 3×32×32）
    x = torch.randn(1, 3, 32, 32)  # 批量大小1，通道3，尺寸32×32
    epoch = 100  # 当前训练轮数
    hw_range = [1, 18]  # 高度和宽度范围

    # 前向传播
    output = arconv(x)

    # 检查输出形状
    print("Output shape:", output.shape)  # 应为 [1, 64, 32, 32]
