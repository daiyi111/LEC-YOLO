...
__all__ = ['C3k2_DFF_1', 'C3k2_DFF_2','Improved_C3k2_DFF']
...
class Improved_DFF(nn.Module):
    def __init__(self, dim):
        ...
        self.dwconv2=nn.Sequential(
            nn.Conv2d(2*dim, dim, kernel_size=3, stride=1, padding=1, groups=dim),
            nn.Conv2d(dim,dim,kernel_size=1,stride=1),
        )
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
    def forward(self, x, skip):
        output1=x+skip
        space_att = self.dwconv1(output1)
        output2 = torch.cat([x, skip], dim=1)
        output2=self.dwconv2(output2)
        channel_att = self.avg_pool(output2)
        output2 = output2 * channel_att
        output2 = output2 * space_att
        return output2
...
class Improved_C3k2_DFF(C2f):
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        ...
        self.cv2 = Conv((2 + n) * self.c, self.c, 1)  #这里必须保证c2=2*c1
        self.cv3=Conv(self.c,c2,1)
        self.improved_DFF=Improved_DFF(self.c)
    ...
    def forward(self, x):
        ...
        return self.cv3(self.improved_DFF(x,y))
